...
class DiffusionInferer(Inferer):
    ...
    def __call__(  # type: ignore[override]
        ...
        alpha_bar_t = self.scheduler.alphas_cumprod[timesteps].view(-1, 1, 1, 1)  # shape: (N, 1, 1, 1)
        beta_bar_t = 1 - alpha_bar_t
        denoised_image: torch.Tensor = (noisy_image - torch.sqrt(beta_bar_t) * prediction) / torch.sqrt(alpha_bar_t)
